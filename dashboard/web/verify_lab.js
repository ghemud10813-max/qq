...
(async function main() {
  ...
  function onData(d) {
    ...
    dataAt = now;
    $('sub').textContent = `${d.n} elements · τ = ${(+d.threshold).toFixed(2)} · seed ${d.seed}`;
    $('rows').innerHTML = [
    ...
    $('note').innerHTML = '<span class="h">HOW IT IS DECIDED</span>each element is measured in its own Pauli basis; it matches when the measured eigenvalue equals the key’s. score = matches / n, accepted when score ≥ τ.' +
