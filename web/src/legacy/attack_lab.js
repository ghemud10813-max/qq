...
function theory(c) {
  // The QVeris 2 app passes the rules of the selected catalog attack directly.
  if (Array.isArray(c.lines) && c.lines.length) return { k: c.k != null ? c.k : Math.round(c.n * (c.hit_fraction || 0)), lines: c.lines };
...
(async function main() {
  ...
  function renderHud() {
    ...
    $('sub').textContent = `${c.name || TYPE_NAMES[c.type] || c.type}${c.type !== 'NONE' ? ` · I = ${(+c.intensity).toFixed(2)}` : ''} · n = ${c.n}`;
    ...
    if (!res) return;
    const L = res.labels || {};
    [['m-anom', L.anomaly], ['m-mis', L.mismatch], ['m-ver', L.verification]].forEach(([id, t]) => { if (t) $(id).querySelector('.row').firstChild.textContent = t + ' '; });
    ...
    vEl.querySelector('b').textContent = res.verdict_label || res.classification || '—';
    vEl.querySelector('span').textContent = res.verdict_sub || { THREAT: 'attack detected and contained', SUSPICIOUS: 'flagged for review', CLEAR: 'session verified', MISSED: 'attack passed undetected' }[V];
  ...
  function hover() {
    ...
    if (slotHit) {
      ...
      if (P) html = `<b>Element ${Math.floor((i * c.n) / Math.max(1, K))} · ${prettyKet(P.label)}</b><span>${isMeasured() ? 'measured' : 'predicted'}</span>` +
        `<p>p(+1) = ${P.pp.toFixed(3)} · ${P.match ? 'verifies' : 'MISMATCH'}${P.affected && c.type !== 'NONE' ? ' · hit by ' + (c.name || TYPE_NAMES[c.type] || c.type) : ''}<br>|b| = ${P.rLen.toFixed(3)}</p>`;
    } else if (eve.group.visible && ray.intersectObject(eve.group, true).length) {
      html = `<b>Eve</b><span>${cfg.name || TYPE_NAMES[cfg.type] || ''}</span><p>${theory(isMeasured() ? res.config : cfg).lines[0]}</p>`;
    } else if (ray.intersectObject(bob.group, true).length) {
      html = res ? `<b>Bob · verifier</b><span>${res.classification}</span><p>${(res.labels && res.labels.anomaly) || 'anomaly'} ${fmt(res.anomaly)} vs ${(res.labels && res.labels.threshold) || 'τ'} ${fmt(res.threshold)}</p>` : '<b>Bob · verifier</b><p>measures each element in its own basis</p>';
    } else if (ray.intersectObject(alice.group, true).length) {
      html = '<b>Alice · signer</b><p>prepares one six-state key qubit per position and teleports it</p>';
    }
    tip.innerHTML = html;
  ...
  function drawDoc(V) {
    ...
    const L = r.labels || {};
    const rows = r.doc_rows || [
      ['decision', r.verdict_label || r.classification],
      ['attack', (c.name || TYPE_NAMES[c.type]) + (c.type !== 'NONE' ? ` · I = ${(+c.intensity).toFixed(2)}` : '')],
      [L.anomaly || 'anomaly score', `${fmt(r.anomaly)}  vs ${L.threshold || 'τ'} ${fmt(r.threshold)}`],
      ['mismatches', `${mis} / ${n}   (${fmt(r.mismatch_rate, 3)})`],
      [L.verification || 'verification', fmt(r.verification)],
    ];
    ...
    if (V === 'MISSED') ev.unshift(r.missed_note || `The attack touched ${theory(c).k} of ${n} elements and the statistics stayed inside the calibrated baseline.`);
