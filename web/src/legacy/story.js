...
const CHAPTERS = [
  ...
  { id: 'bind', t: [3.5, 7], num: 'Chapter I', title: 'Bind',
    film: 'SHA-256 binds every byte of the signed envelope.',
    body: 'The message, signer, recipients, sequence number, timestamp and a fresh nonce form an envelope. Its SHA-256 digest is what gets signed: change one character and every downstream state changes with it.',
    code: 'sentinel/protocol/encoding.py' },
  { id: 'sign', t: [7, 11], num: 'Chapter II', title: 'Sign',
    film: 'Each digest bit opens one of two one-time quantum keys.',
    body: 'Alice holds two one-time keys per digest bit (Lamport style). Each key is a string of random six-state qubits: |0⟩ |1⟩ |+⟩ |−⟩ |+i⟩ |−i⟩. Signing reveals the key chosen by each bit. A bundle signs exactly once.',
    code: 'sentinel/protocol/keys.py · signing.py' },
  { id: 'entangle', t: [11, 15], num: 'Chapter III', title: 'Entangle',
    film: 'Bell pairs |Φ+⟩ link Alice to each verifier. CHSH certifies them first.',
    body: 'Before a single key qubit is trusted, sacrificial Bell pairs play the CHSH game. Only real entanglement beats the classical bound S ≤ 2; the lower confidence bound must clear it, or the link is refused.',
    code: 'sentinel/bell.py · detection D1–D3' },
  { id: 'teleport', t: [15, 19.5], num: 'Chapter IV', title: 'Teleport',
    film: 'Bell measurement, 2 classical bits, Pauli correction. The key state reappears at Bob.',
    body: 'Every key qubit is teleported: a Bell measurement, two classical bits, an X/Z correction. Bob measures in a random basis and keeps records he cannot forge. Bob and Charlie then swap half their records (symmetrization) so neither can be singled out.',
    code: 'sentinel/teleport.py · protocol/distribution.py' },
  { id: 'intrude', t: [19.5, 23], num: 'Chapter V', title: 'Intrusion',
    film: 'Eve strikes: 17 attacks across forgery, impersonation, replay, interception, channel tampering and repudiation.',
    body: 'Eve can tamper with the fibre, flip the classical frame bits, harvest keys man-in-the-middle, forge with or without insider knowledge, impersonate a signer, replay old signatures, or help a signer prepare a dispute.',
    code: 'sentinel/adversary/catalog.py' },
  { id: 'detect', t: [23, 27], num: 'Chapter VI', title: 'Detect',
    film: 'QSentinel reads the physics: CHSH, QBER, de-twirled tomography, SPRT. No ML.',
    body: 'Twenty detectors watch every run: Bell certificates, per-basis error rates, a tomography that undoes teleportation’s Pauli twirl to fingerprint the channel, frame integrity, sequential tests and a protocol guard. Exact statistics decide; the evidence names the attack.',
    code: 'sentinel/detection/ · fusion.py' },
...
function buildWorld(stage) {
  ...
  new FontLoader().load(QV.vendor ? `${QV.vendor}/examples/fonts/helvetiker_bold.typeface.json` : `https://cdn.jsdelivr.net/npm/three@${QV.three_version || '0.165.0'}/examples/fonts/helvetiker_bold.typeface.json`, (font) => {
...
(async function main() {
  ...
  // Host-page listeners are tracked so an SPA host can unmount the scene cleanly.
  const hostListeners = [], hostTimers = [];
  const Padd = (...a) => { try { P.addEventListener(...a); hostListeners.push(a); } catch (e) {} };
  window.addEventListener('pagehide', () => {
    hostListeners.forEach((a) => { try { P.removeEventListener(...a); } catch (e) {} });
    hostTimers.forEach(clearInterval);
    try { const b = pdoc && pdoc.getElementById('qv-back'); if (b) b.remove(); pdoc.documentElement.style.removeProperty('--qv-hero-h'); } catch (e) {}
  });
  ...
  if (m) {
    ...
    $('finale-metrics').innerHTML = m.chips
      ? m.chips.map(([k, v]) => chip(k, v)).join('')
      : chip('detection rate', pc(m.detection_rate)) + chip('false acceptance', pc(m.far)) +
        chip('false rejection', pc(m.frr)) + chip('threshold τ', m.threshold != null ? Number(m.threshold).toFixed(4) : 'n/a');
    $('finale-src').textContent = m.source || 'measured · experiments/results/final';
  }
  ...
  if (embedded) {
    ...
    layoutDock();
    Padd('resize', () => { if (mode === 'dock') layoutDock(); });
    ...
    Padd('scroll', syncBack, true);
    hostTimers.push(setInterval(syncBack, 800));
  ...
  try { P && Padd('keydown', (e) => { if (mode === 'film' && e.key === 'Escape') dock(); }); } catch (e) {}
