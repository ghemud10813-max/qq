...
const NOISE = {
  ...
  dephasing: { name: 'Dephasing', glyph: 'Z', formula: (p) => `phase randomised with p = ${p.toFixed(2)} · (x, y, z) → ((1−p)x, (1−p)y, z)` },
  bit_phase_flip: { name: 'Bit-phase flip', glyph: 'Y', formula: (p) => `Y applied with probability p = ${p.toFixed(2)} · (x, y, z) → ((1−2p)x, y, (1−2p)z)` },
  phase_damping: { name: 'Phase damping', glyph: '', formula: (p) => `coherence decays with λ = ${p.toFixed(2)} · (x, y) → √(1−λ)(x, y), z → z` },
};
...
function channelMap(type, p) {
  switch (type) {
    ...
    case 'dephasing': return { s: [1 - p, 1 - p, 1], dz: 0 };
    case 'bit_phase_flip': return { s: [Math.abs(1 - 2 * p), 1, Math.abs(1 - 2 * p)], dz: 0 };
    case 'phase_damping': return { s: [Math.sqrt(1 - p), Math.sqrt(1 - p), 1], dz: 0 };
...
(async function main() {
  ...
  function onData(d) {
    ...
    $('note').innerHTML = D.note || `<span class="h">CHANNEL · ${nz.name.toUpperCase()}</span>${nz.formula(+D.p)}<br>ellipsoid: one application (theory) · coral arrow: measured ρ<sub>Bob</sub> after the full teleportation circuit`;
    const b = $('badge');
